        ...
        >
            ...
            >
                <template slot-scope="scope">
                    <router-link
                        :to="{name: 'Show Cliente', params: {id: scope.row.id}}"
...
import {CLIENTE_FETCH, CLIENTE_OBTAIN} from '../store/types'
...
export default {
    ...
    data() {
        return {
            ...
            page: 1,
            formVisible: false,
            formTitle: 'New Cliente',
            formData: null
        }
    },
    computed: {
        ...mapGetters(['clientes', 'clientesMeta', 'clientesLoading']),
    },
    created() {
        this.fetchData()
    },
    ...
    methods: {
        ...mapActions([CLIENTE_FETCH]),
        ...mapMutations([CLIENTE_OBTAIN]),
        ...
        fetchData() {
            ...
            this[CLIENTE_FETCH](params)
        },
        handleAdd() {
            this.formTitle = 'New Cliente'
            this.formData = {}
            this.formVisible = true
        },
        handleEdit(row) {
            this.formTitle = 'Edit Cliente'
            this.formData = Object.assign({}, row)
            this.formVisible = true
        },
        handleDelete(row) {
            this.$confirm('This will permanently delete the cliente. Continue?', 'Warning', {
                type: 'warning'
            }).then(() => {
                clienteApi.delete(row.id).then((response) => {
                    this.$message({
